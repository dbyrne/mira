...
That writes `output/<config>/emission/` (`dso_plan.md` + `.csv` + `nina_targets.csv`, same DSO report). `nina_targets.csv` is a true **NINA Target Scheduler target-import** file (canonical names → capture sidecars stay ledger-matched; Rotation = the catalog's optional `pa_deg` from the image books, CAA-ready; exposure plans still get attached in the TS UI from the budget tables). The plan markdown's footer carries rig-aware dither guidance — load-bearing because TS dither **silently no-ops without a guider** (S30 → connect NINA's Direct Guider, or use `mira capture`'s guider-less slew-dither; one conductor per night, never both). The curated catalog (`data/dso_catalog/emission_nebulae.yaml`, ~40 emission targets, union of the Esprit 120 + Esprit 80 + S30 image books under `output/books/{esprit,esprit80,s30}_emission_book/`) is rig-agnostic — the per-rig `emission:` config section sets `fov_deg`, so the giant complexes (North America, Heart) are mosaic-flagged on the Esprit 120 single frame but fit one Esprit 80 / S30 frame. `output/books/rig_fit_matrix.md` (regen: `python output/books/make_rig_matrix.py`) is the cross-rig one-pager: per-target fit class on all three rigs + a most-resolution-that-frames-it verdict.
...
- `mira stack --register {auto,stars,wcs}` controls frame alignment (`siril_pipeline.run_siril_stack`). `stars` = Siril Global Star Alignment only. `wcs` = `wcs_register_stack` (translation-align by plate-solved WCS + sigma-clipped mean; needs solved frames, applies no flat/dark calibration; STF-autostretch preview). `auto` (default) runs Siril GSA then **falls back to WCS** when GSA fails and the frames carry a WCS — the fix for emission-**filled** fields (NGC 7000, IC 1396, Heart) where Siril's star detection finds too few point sources. The 2026-06-17 NGC 7000 lesson is baked in: a stack/registration failure is a *pipeline* signal (bad reference / too few stars), **never a data-quality verdict**. Pinned by `tests/test_wcs_stack.py`. `--lights` accepts **multiple session dirs** → co-stack: their frames are recomposed into an ephemeral temp dir for the run (`_gather_lights` — hardlinked, never persisted or backed up), so a standing `_combined` dir under `captures/` is unnecessary (e.g. `mira stack --lights captures/ngc6888/s30_lp_20260530 captures/ngc6888/s30_lp_20260601 --out …`).
