"""statistical -- median/anchor-targeting auto-stretch (MTF), re-tuned for M97 (Owl Nebula).

Seti Astro "Statistical Stretch" / PixInsight & Siril unlinked STF AutoStretch, adapted
for the M97 FAINT-PLANETARY / chroma regime (rim COLOR retention, not faint lift).

  -- WHAT CHANGED FOR M97 (vs the M57 Ring port) ------------------------------------------
  Same engine (linked MTF + noise-suppressing toe), re-anchored. M97 is a much fainter,
  lower-surface-brightness planetary than M57: on this master the rim signal lands at
  x ~ 0.05 in the harness's normalized space (rim lum median 0.049, p1 0.027, max 0.077),
  NOT at M57's ~0.18. The OIII-teal disk dominates with a weaker Ha-red component
  (per-channel rim medians on this master: R 0.027, G 0.057, B 0.065 -> blue-green body).

  Carrying M57's anchor=0.18 onto M97 was the trap (verified): the real rim sits a quarter
  of the way down the input axis from where the steep MTF slope is placed, so the rim never
  reaches the spreading part of the curve -- it gets COMPRESSED. Result: rim_lum ~0.11-0.15
  (less than HALF the asinh-0.03 baseline's 0.29, far outside the +-15% band) and rim_chroma
  ~0.21 (BELOW baseline 0.31). The fix is to drop the anchor onto M97's actual rim level
  (~0.05) so the slope>1 region lands on the rim and SPREADS its channels (chroma grows)
  while the MTF shoulder still holds the brightest disk pixels under 1.0 (rim_clip ~ 0).

  -- THE TOE (noise lever), re-tuned ------------------------------------------------------
  Because the anchor is now low, the toe_knee MUST drop with it. The rule is unchanged but
  the numbers are M97's: the knee stays BELOW the rim's lower tail so the foot only crushes
  sky scatter, never rim signal. On this master rim p1=0.027, p5=0.033, so toe_knee=0.030
  sits at/below the faintest ~1% of rim pixels (noise-dominated edge) and leaves the rim
  body fully intact. ADVERSARIAL NOTE (the gaming trap, verified by eye): pushing the knee
  UP into the rim tail (tk>=0.036, above the rim p10=0.036) and the power up (g>=4) posts a
  much larger rim_chroma (~0.48) and a tiny sky_noise_chroma -- but it's DISHONEST. It earns
  the number by darkening the faint rim margin (manufacturing a hard over-saturated blue edge
  ring) and clipping the outer halo to black (halo_contrast collapses 0.006 -> 0.001). The
  metric rewards it; the eye shows an over-processed disk with a fake rim. So the honest
  ceiling is toe_knee at/below the rim's ~1st percentile (<=0.030 here) with a moderate power.

  -- M57 history (engine rationale, still valid) ------------------------------------------
  M57's rim sat at x~0.15-0.6 (anchor 0.18 -> target 0.40, m~0.25, slope>1 across the rim,
  shoulder protecting knots). The M51 faint-galaxy mistake before THAT was anchoring on the
  ~1e-3 NOISE floor (m~0.001, the whole rim slammed to >0.98 flat white, chroma collapsed).
  Same lesson each time: anchor on the SIGNAL of interest at its actual normalized scale.
...
#
# DEFAULTS = the HONEST winner for M97: anchor on the actual rim level (~0.05), target 0.35
# (rim slightly brighter than baseline but inside the +-15% band), toe_knee 0.030 sitting at
# the rim's ~1st percentile (crushes sky scatter, leaves the rim body intact), moderate power.
# Measured on this master (asinh a=0.03 baseline in []):
#   rim_chroma 0.4136 [0.3132]  +32%, the honest multi-hue gain (OIII teal + Ha red both up)
#   rim_lum    0.3297 [0.2917]  +13%, INSIDE the +-15% band (do-not-dim satisfied)
#   rim_clip   0.0    [0.0]     no blown rim
#   center_chroma 0.4801 [0.367]
#   sky_noise_lum    0.0242 [0.0292]  strictly under baseline
#   sky_noise_chroma 0.0157 [0.0391]  strictly under baseline (toe footing the sky mottle)
#   halo_contrast 0.006 (faint -- lin_halo_snr is only 0.38, so NOT chased; halo stays >= sky)
# The eye check confirms a coherent teal disk with discernible cavities + clean dark sky.
# The higher-chroma rows (tk>=0.036) are DELIBERATELY excluded from DEFAULT -- see docstring:
# they game the metric by eating the rim's faint margin (fake blue edge ring) + clipping halo.
DEFAULTS = {"anchor": 0.05, "target": 0.35, "toe_knee": 0.030, "toe_pow": 3.0, "black": 0.0}

SWEEP = [
    {"anchor": 0.05, "target": 0.35, "toe_knee": 0.030, "toe_pow": 3.0, "black": 0.0},  # DEFAULT: honest winner (rc 0.4136, rl 0.3297 in-band, both noise gates strictly under)
    {"anchor": 0.05, "target": 0.35, "toe_knee": 0.030, "toe_pow": 2.0, "black": 0.0},  # weaker toe -> a touch more sky mottle, slightly less chroma (rc 0.3961, snc 0.0227)
    {"anchor": 0.055, "target": 0.33, "toe_knee": 0.030, "toe_pow": 3.0, "black": 0.0}, # rim_lum dead-on baseline (rl ~0.292), most conservative brightness, rc ~0.378
    {"anchor": 0.045, "target": 0.35, "toe_knee": 0.030, "toe_pow": 3.0, "black": 0.0}, # higher gain: rim near +15% band edge, more chroma (rc ~0.43) -- watch rim_lum
    {"anchor": 0.05, "target": 0.36, "toe_knee": 0.036, "toe_pow": 4.0, "black": 0.0},  # ADVERSARIAL: rc 0.4824 but toe eats rim margin (fake edge ring) + clips halo -- NOT honest
    {"anchor": 0.05, "target": 0.35, "toe_knee": 0.0, "toe_pow": 1.0, "black": 0.0},    # NO toe (plain STF): noise UP, chroma DOWN -- shows why the toe matters
]
...
def apply(x, anchor=0.05, target=0.35, toe_knee=0.030, toe_pow=3.0, black=0.0):
    anchor = float(anchor)
    target = float(target)
