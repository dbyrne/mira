# GH Dra

Score: **102.0**  
Observable from: **Fairbanks + Jersey City**

## Catalog

- VSX type: `LB:`
- Coordinates: RA `248.89954`, Dec `61.84683`
- Catalog photometry: range `10.040-10.190` (Hp/Hp)
- Catalog amplitude: `0.150` mag
- Period: `` days
- Spectral type: `M0`
- Galactic latitude: `39.5 deg`
- VSX: https://www.aavso.org/vsx/index.php?view=detail.top&oid=13863
- AAVSO finder chart: https://apps.aavso.org/vsp/photometry/?star=GH+Dra&type=chart&fov=900&maglimit=15&resolution=150&north=up&east=left

## Observability from Fairbanks (best)

- Max altitude in dark window: `67.3 deg`
- Best single-night dark time above altitude floor: `420 min`
- Best window date: `2026-09-18`
- Best sampled local time: `2026-09-18T22:00:00-08:00`

## Observability from Jersey City

- Max altitude in dark window: `60.2 deg`
- Best single-night dark time above altitude floor: `150 min`
- Best window date: `2026-09-18`
- Best sampled local time: `2026-09-18T20:00:00-04:00`

## Observing Strategy

- Long-cadence follow-up: one calibrated point every clear night or two is useful; weekly cadence is still worthwhile for slow red variables.

## Why It Was Flagged

- max altitude 67.3 deg from Fairbanks
- long nightly window from Fairbanks
- uncertain or broad VSX type (LB:)
- classical GCVS variable, suitable for practice and follow-up
- modest catalog amplitude about 0.15 mag
- bright enough for Fairbanks (10.04)
- no catalog period listed
- well away from Galactic plane (b=39.5 deg)
- also observable from Jersey City
- sparse AAVSO coverage (0 recent observations)
- Gaia ipd_frac_multi_peak=3.00 suggests blended/contaminated PSF

## AAVSO Recent Coverage

- Status: `ok`
- Recent observations: `0`
- JD range: `2460435.08-2461165.08`
- Most recent observation: `none in the configured window`

## SIMBAD Context

- Status: `ok`
- Main ID: `V* GH Dra`
- Object type: `LP?`
- Match separation: `0.014` arcsec
- Search: https://simbad.cds.unistra.fr/simbad/sim-coo?Coord=248.899540+61.846830&Radius=5.0&Radius.unit=arcsec
- Other IDs: `TIC 198237255`, `HIP 81244`, `Gaia DR3 1625465733638160000`, `AG+61  948`, `BD+62  1491`, `GSC 04190-00685`, `HIC  81244`, `IRAS 16349+6156`

## Gaia DR3 Context

- Status: `ok`
- Source ID: `1625465733638160000`
- G magnitude: `8.979`
- BP-RP color: `2.529`
- Parallax: `0.677` +/- `0.016` mas
- RUWE: `1.070`
- Gaia photometric variability flag: `not flagged`
- Match separation: `0.055` arcsec
- IPD multi-peak fraction: `3.000` (PSF appears blended/contaminated)

## ZTF Enrichment

- Not requested for this run.

## Human Review Checklist

- Check VSX and SIMBAD for newer notes or duplicate names.
- Inspect DSS/Pan-STARRS imagery for crowding and bright nearby stars.
- Verify AAVSO comparison stars are available in the field.
- Decide cadence: single nightly point, weekly monitoring, or continuous time-series.
- Treat this as a follow-up candidate, not a discovery claim.
