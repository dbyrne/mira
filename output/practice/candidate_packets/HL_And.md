# HL And

Score: **98.0**  
Observable from: **Fairbanks + Jersey City**

## Catalog

- VSX type: `SRB`
- Coordinates: RA `14.48517`, Dec `38.45150`
- Catalog photometry: range `12.000-14.200` (pg/pg)
- Catalog amplitude: `2.200` mag
- Period: `100.00000000` days
- Spectral type: `blank`
- Galactic latitude: `-24.4 deg`
- VSX: https://www.aavso.org/vsx/index.php?view=detail.top&oid=212
- AAVSO finder chart: https://apps.aavso.org/vsp/photometry/?star=HL+And&type=chart&fov=900&maglimit=15&resolution=150&north=up&east=left

## Observability from Fairbanks (best)

- Max altitude in dark window: `63.6 deg`
- Best single-night dark time above altitude floor: `420 min`
- Best window date: `2026-09-24`
- Best sampled local time: `2026-09-25T02:30:00-08:00`

## Observability from Jersey City

- Max altitude in dark window: `74.5 deg`
- Best single-night dark time above altitude floor: `180 min`
- Best window date: `2026-09-21`
- Best sampled local time: `2026-09-22T00:30:00-04:00`

## Observing Strategy

- Long-cadence follow-up: one calibrated point every clear night or two is useful; weekly cadence is still worthwhile for slow red variables.

## Why It Was Flagged

- max altitude 74.5 deg from Jersey City
- long nightly window from Jersey City
- classical GCVS variable, suitable for practice and follow-up
- catalog amplitude about 2.20 mag
- bright enough for Jersey City (12.00)
- long-period cadence friendly (100.00 d)
- well away from Galactic plane (b=-24.4 deg)
- also observable from Fairbanks
- sparse AAVSO coverage (0 recent observations)

## AAVSO Recent Coverage

- Status: `ok`
- Recent observations: `0`
- JD range: `2460435.08-2461165.08`
- Most recent observation: `none in the configured window`

## SIMBAD Context

- Status: `ok`
- Main ID: `V* HL And`
- Object type: `LP*`
- Match separation: `0.586` arcsec
- Search: https://simbad.cds.unistra.fr/simbad/sim-coo?Coord=14.485170+38.451500&Radius=5.0&Radius.unit=arcsec
- Other IDs: `TIC 432843785`, `Gaia DR3 370775494348664192`, `CSV   5881`, `GSC 02798-00372`, `SV* WR     5`, `V* HL And`, `2MASS J00575647+3827051`, `TYC 2798-372-1`

## Gaia DR3 Context

- Status: `ok`
- Source ID: `370775494348664192`
- G magnitude: `10.330`
- BP-RP color: `1.844`
- Parallax: `0.155` +/- `0.017` mas
- RUWE: `1.124`
- Gaia photometric variability flag: `VARIABLE`
- Match separation: `0.658` arcsec
- IPD multi-peak fraction: `0.000`

## ZTF Enrichment

- Not requested for this run.

## Human Review Checklist

- Check VSX and SIMBAD for newer notes or duplicate names.
- Inspect DSS/Pan-STARRS imagery for crowding and bright nearby stars.
- Verify AAVSO comparison stars are available in the field.
- Decide cadence: single nightly point, weekly monitoring, or continuous time-series.
- Treat this as a follow-up candidate, not a discovery claim.
