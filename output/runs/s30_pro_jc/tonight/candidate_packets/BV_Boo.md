# BV Boo

Score: **98.0**  
Observable from: **Jersey City**

## Catalog

- VSX type: `LB`
- Coordinates: RA `205.51611`, Dec `22.91378`
- Catalog photometry: range `11.700-12.100` (V/V)
- Catalog amplitude: `0.400` mag
- Period: `37.62012000` days
- Spectral type: `blank`
- Galactic latitude: `77.8 deg`
- VSX: https://www.aavso.org/vsx/index.php?view=detail.top&oid=4402
- AAVSO finder chart: https://apps.aavso.org/vsp/photometry/?star=BV+Boo&type=chart&fov=900&maglimit=15&resolution=150&north=up&east=left

## Observability from Jersey City (best)

- Max altitude in dark window: `72.2 deg`
- Best single-night dark time above altitude floor: `180 min`
- Best window date: `2026-05-30`
- Best sampled local time: `2026-05-30T22:00:00-04:00`

## Observing Strategy

- Long-cadence follow-up: one calibrated point every clear night or two is useful; weekly cadence is still worthwhile for slow red variables.

## Why It Was Flagged

- max altitude 72.2 deg from Jersey City
- long nightly window from Jersey City
- classical GCVS variable, suitable for practice and follow-up
- catalog amplitude about 0.40 mag
- bright enough for Jersey City (11.70)
- long-period cadence friendly (37.62 d)
- well away from Galactic plane (b=77.8 deg)
- sparse AAVSO coverage (0 recent observations)

## AAVSO Recent Coverage

- Status: `ok`
- Recent observations: `0`
- JD range: `2460461.41-2461191.41`
- Most recent observation: `none in the configured window`

## SIMBAD Context

- Status: `ok`
- Main ID: `V* BV Boo`
- Object type: `LP*`
- Match separation: `0.260` arcsec
- Search: https://simbad.cds.unistra.fr/simbad/sim-coo?Coord=205.516110+22.913780&Radius=5.0&Radius.unit=arcsec
- Other IDs: `Gaia DR3 1443518893070179712`, `GSC 01998-01020`, `SV* SON  8501`, `V* BV Boo`, `2MASS J13420387+2254497`, `TYC 1998-1020-1`, `TIC 417137018`, `Gaia DR1 1443518888775544064`

## Gaia DR3 Context

- Status: `ok`
- Source ID: `1443518893070179712`
- G magnitude: `11.329`
- BP-RP color: `1.682`
- Parallax: `0.093` +/- `0.026` mas
- RUWE: `1.344`
- Gaia photometric variability flag: `VARIABLE`
- Match separation: `0.152` arcsec
- IPD multi-peak fraction: `0.000`

## ZTF Enrichment

- Not requested for this run.

## Human Review Checklist

- Check VSX and SIMBAD for newer notes or duplicate names.
- Inspect DSS/Pan-STARRS imagery for crowding and bright nearby stars.
- Verify AAVSO comparison stars are available in the field.
- Decide cadence: single nightly point, weekly monitoring, or continuous time-series.
- Treat this as a follow-up candidate, not a discovery claim.
