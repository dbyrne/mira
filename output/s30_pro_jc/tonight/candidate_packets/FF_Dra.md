# FF Dra

Score: **62.8**  
Observable from: **Jersey City**

## Catalog

- VSX type: `LB`
- Coordinates: RA `162.88901`, Dec `75.19108`
- Catalog photometry: range `8.670-8.900` (Hp/Hp)
- Catalog amplitude: `0.230` mag
- Period: `` days
- Spectral type: `M2`
- Galactic latitude: `39.6 deg`
- VSX: https://www.aavso.org/vsx/index.php?view=detail.top&oid=13842
- AAVSO finder chart: https://apps.aavso.org/vsp/photometry/?star=FF+Dra&type=chart&fov=900&maglimit=15&resolution=150&north=up&east=left

## Observability from Jersey City (best)

- Max altitude in dark window: `50.8 deg`
- Best single-night dark time above altitude floor: `120 min`
- Best window date: `2026-05-30`
- Best sampled local time: `2026-05-30T22:00:00-04:00`

## Observing Strategy

- Long-cadence follow-up: one calibrated point every clear night or two is useful; weekly cadence is still worthwhile for slow red variables.

## Why It Was Flagged

- max altitude 50.8 deg from Jersey City
- usable nightly window from Jersey City
- classical GCVS variable, suitable for practice and follow-up
- modest catalog amplitude about 0.23 mag
- bright enough for Jersey City (8.67)
- no catalog period listed
- well away from Galactic plane (b=39.6 deg)
- sparse AAVSO coverage (0 recent observations)

## AAVSO Recent Coverage

- Status: `ok`
- Recent observations: `0`
- JD range: `2460461.41-2461191.41`
- Most recent observation: `none in the configured window`

## SIMBAD Context

- Status: `ok`
- Main ID: `V* FF Dra`
- Object type: `LP*`
- Match separation: `0.005` arcsec
- Search: https://simbad.cds.unistra.fr/simbad/sim-coo?Coord=162.889010+75.191080&Radius=5.0&Radius.unit=arcsec
- Other IDs: `TIC 142504227`, `HIP 53079`, `Gaia DR3 1080096593187951104`, `AG+75  500`, `BD+75   426`, `GSC 04549-01700`, `HIC  53079`, `IRAS 10476+7527`

## Gaia DR3 Context

- Status: `ok`
- Source ID: `1080096593187951104`
- G magnitude: `7.468`
- BP-RP color: `2.852`
- Parallax: `1.194` +/- `0.019` mas
- RUWE: `0.989`
- Gaia photometric variability flag: `VARIABLE`
- Match separation: `0.299` arcsec
- IPD multi-peak fraction: `0.000`

## ZTF Enrichment

- Not requested for this run.

## Human Review Checklist

- Check VSX and SIMBAD for newer notes or duplicate names.
- Inspect DSS/Pan-STARRS imagery for crowding and bright nearby stars.
- Verify AAVSO comparison stars are available in the field.
- Decide cadence: single nightly point, weekly monitoring, or continuous time-series.
- Treat this as a follow-up candidate, not a discovery claim.
