    ...
    for row in reader:
        ...
        targets.append(
            VsxTarget(
                ...
                bright_mag=_parse_float(row.get("max")),
                faint_mag=_parse_float(row.get("min")),
                bright_band=(row.get("n_max") or "").strip(),
                faint_band=(row.get("n_min") or "").strip(),
                faint_is_amplitude=bool((row.get("f_min") or "").strip()),
